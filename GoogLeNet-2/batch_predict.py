...
def batch_predict(model, folder_path, transform, classes, device):
    ...
    supported_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.jfif')
    ...
    for img_name in os.listdir(folder_path):
        if img_name.lower().endswith(supported_extensions):
            ...
            try:
                ...
                new_file_name = f"{pred_class}{file_root[file_root.find('_'):] if '_' in file_root else ''}{file_ext}"
...
if __name__ == "__main__":
    ...
    # 图像预处理方式（必须与训练时一致）
    normalize = transforms.Normalize([0.22890568, 0.19639583, 0.1433638],
                                     [0.09950783, 0.07997292, 0.06596899])
